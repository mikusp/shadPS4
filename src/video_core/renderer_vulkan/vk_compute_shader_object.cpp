...
ComputeShaderObject::ComputeShaderObject(const Instance& instance, Scheduler& scheduler,
                                 DescriptorHeap& desc_heap, const Shader::Profile& profile,
                                 vk::PipelineCache pipeline_cache, ComputePipelineKey compute_key_,
                                 const Shader::Info& info_, vk::ShaderEXT shader,
                                 SerializationSupport& sdata, bool preloading /*=false*/)
    : ShaderObject{instance, scheduler, desc_heap, profile, pipeline_cache, true},
      compute_key{compute_key_} {
    // auto& info = stages[int(Shader::LogicalStage::Compute)];
    info = &info_;
    const auto debug_str = GetDebugString();

    shader_object = std::move(vk::UniqueShaderEXT{
        shader,
        vk::detail::ObjectDestroy<vk::Device, VULKAN_HPP_DEFAULT_DISPATCHER_TYPE>(instance.GetDevice())
    });
...
ComputeShaderObject::~ComputeShaderObject() = default;
