...
const Shader::RuntimeInfo& ShaderObjectCache::BuildRuntimeInfo(Stage stage, LogicalStage l_stage) {
...
ShaderObjectCache::ShaderObjectCache(const Instance& instance_, Scheduler& scheduler_,
...
ShaderObjectCache::~ShaderObjectCache() = default;

const GraphicsPipeline* ShaderObjectCache::GetGraphicsPipeline() {
...
const ComputeShaderObject* ShaderObjectCache::GetComputeShaderObject() {
    if (!RefreshComputeKey()) {
        return nullptr;
    }
    const auto [it, is_new] = compute_shader_objects.try_emplace(compute_key);
    if (is_new) {
        const auto pipeline_hash = std::hash<ComputePipelineKey>{}(compute_key);
        LOG_INFO(Render_Vulkan, "Compiling compute shader object {:#x}", pipeline_hash);

        ComputeShaderObject::SerializationSupport sdata{};
        it.value() = std::make_unique<ComputeShaderObject>(instance, scheduler, desc_heap, profile,
                                                       *pipeline_cache, compute_key, *infos[0],
                                                       shaders[0], sdata, false);
...
bool ShaderObjectCache::RefreshGraphicsKey() {
...
bool ShaderObjectCache::RefreshGraphicsStages() {
    const auto& regs = liverpool->regs;
    auto& key = graphics_key;
...
bool ShaderObjectCache::RefreshComputeKey() {
    Shader::Backend::Bindings binding{};
    const auto& cs_pgm = liverpool->GetCsRegs();
    const auto cs_params = AmdGpu::GetParams(cs_pgm);
    std::tie(infos[0], shaders[0], fetch_shader, compute_key.value) =
        GetProgram(Shader::Stage::Compute, LogicalStage::Compute, cs_params, binding);
    return true;
}

vk::ShaderEXT ShaderObjectCache::CompileModule(Shader::Info& info, Shader::RuntimeInfo& runtime_info,
    ...
    const auto stage = LogicalStageToStageBit[std::to_underlying(info.l_stage)];
    ...
    const auto uses_push_descriptors = binding_num < instance.MaxPushDescriptors();
    ...
    if (is_patched) {
        ...
        shader = CompileShaderSPV(*patch, device, stage, &*descriptor_set, &push_constants);
    } else {
        shader = CompileShaderSPV(spv, device, stage, &*descriptor_set, &push_constants);
    }

    RegisterShaderBinary(std::move(spv), info.pgm_hash, perm_idx);

    const auto name = GetShaderName(info.stage, info.pgm_hash, perm_idx);
    Vulkan::SetObjectName(device, shader, name);
    if (EmulatorSettings.IsShaderCollect()) {
        // DebugState.CollectShader(name, info.l_stage, shader, spv, code,
        //                          patch ? *patch : std::span<const u32>{}, is_patched);
    }
    return shader;
}

ShaderObjectCache::Result ShaderObjectCache::GetProgram(Stage stage, LogicalStage l_stage,
                                                const Shader::ShaderParams& params,
                                                Shader::Backend::Bindings& binding) {
    auto runtime_info = BuildRuntimeInfo(stage, l_stage);
    auto [it_pgm, new_program] = program_cache.try_emplace(params.hash);
    if (new_program) {
        it_pgm.value() = std::make_unique<ShaderProgram>(stage, l_stage, params);
        auto& program = it_pgm.value();
        auto start = binding;
        const auto shader = CompileModule(program->info, runtime_info, params.code, 0, binding);
        ...
        program->AddPermut(shader, std::move(spec));
        return std::make_tuple(&program->info, shader, program->objects[0].spec.fetch_shader_data,
                               perm_hash);
    }
    ...
    size_t perm_idx = program->objects.size();
    u64 perm_hash = HashCombine(params.hash, perm_idx);

    vk::ShaderEXT shader{};

    const auto it = std::ranges::find(program->objects, spec, &ShaderProgram::Object::spec);
    if (it == program->objects.end()) {
        auto new_info = Shader::Info(stage, l_stage, params);
        shader = CompileModule(new_info, runtime_info, params.code, perm_idx, binding);
        ...
        shader = it->object;
        perm_idx = std::distance(program->objects.begin(), it);
        perm_hash = HashCombine(params.hash, perm_idx);
    }
    return std::make_tuple(&program->info, shader,
                           program->objects[perm_idx].spec.fetch_shader_data, perm_hash);
}

std::optional<vk::ShaderModule> ShaderObjectCache::ReplaceShader(vk::ShaderModule module,
...
std::string ShaderObjectCache::GetShaderName(Shader::Stage stage, u64 hash,
...
void ShaderObjectCache::DumpShader(std::span<const u32> code, u64 hash, Shader::Stage stage,
...
std::optional<std::vector<u32>> ShaderObjectCache::GetShaderPatch(u64 hash, Shader::Stage stage,
                                                              size_t perm_idx,
                                                              std::string_view ext) {
