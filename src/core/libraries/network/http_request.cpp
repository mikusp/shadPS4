...
int ConvertCurlCodeToOrbis(CURLcode e) {
    ...
    case CURLE_COULDNT_CONNECT:
        return ORBIS_HTTP_ERROR_RESOLVER_ESERVERREFUSED;
    default:
...
HttpRequest::HttpRequest(HttpConnection& connection_) : HttpRequest() {
    nonBlocking = connection_.nonBlocking;
}
...
int HttpRequest::SendBlocking(const void* postData, size_t size) {
    ...
    if (epoll != -1 && (e = h->GetObject<HttpEpoll>(epoll), !e)) {
        return ORBIS_HTTP_ERROR_INVALID_ID;
    }
    ...
    LOG_INFO(Lib_Http, "performing {}", curl_easy_strerror(mc));
    if (mc) {
        LOG_ERROR(Lib_Http, "{}", curl_easy_strerror(mc));
    }
    ...
    //     if (mc) {
    //         LOG_ERROR(Lib_Http, "{}", curl_multi_strerror(mc));
    //     }
    //     if (!mc && still_running) {
