...
const s32 ORBIS_NP_WEBAPI_ERROR_INVALID_LIB_CONTEXT_ID = 0x80552903;
const s32 ORBIS_NP_WEBAPI_ERROR_LIB_CONTEXT_NOT_FOUND = 0x80552904;
const s32 ORBIS_NP_WEBAPI_ERROR_USER_CONTEXT_NOT_FOUND = 0x80552905;
...
constexpr u32 MAX_LIB_CONTEXTS = 8;
constexpr u32 MAX_USER_CONTEXTS = 8;
constexpr u32 MAX_ACTIVE_REQUESTS = 16;
...
struct LibraryContext {
    std::mutex mutex;
    std::array<UserContext*, MAX_USER_CONTEXTS> userContext = {nullptr};
};
struct Request {
    std::mutex mutex;
    UserContext* userCtx;
    std::string api;
    std::string path;
    OrbisNpWebApiHttpMethod httpMethod;
    u64 contentLen;
    std::string contentType;
    std::unique_ptr<httplib::Client> client = nullptr;
    std::optional<httplib::Result> response{};
    u64 responseBodyOffset = 0;
};
struct UserContext {
    std::mutex mutex;
    LibraryContext* libCtx;
    Libraries::UserService::OrbisUserServiceUserId userId;
    std::array<Request*, MAX_ACTIVE_REQUESTS> requests = {nullptr};
};

static std::mutex lib_mutex;
static std::array<LibraryContext*, MAX_LIB_CONTEXTS> libContext = {nullptr};

s32 PS4_SYSV_ABI sceNpWebApiCreateContext(s32 libCtxId,
                                          Libraries::Np::OrbisNpOnlineId* npOnlineId) {
    LOG_ERROR(Lib_NpWebApi, "called, libCtxId = {}", libCtxId);
    ...
    // get that from npOnlineId
    auto userId = 1;

    std::scoped_lock lk{ctx->mutex};
    auto exists = std::ranges::find_if(
        ctx->userContext.begin(), ctx->userContext.end(),
        [userId](auto userCtx) { return userCtx != nullptr && userCtx->userId == userId; });
    if (exists != ctx->userContext.end()) {
        return ORBIS_NP_WEBAPI_ERROR_USER_CONTEXT_ALREADY_EXIST;
    }

    if (auto slot = std::ranges::find(ctx->userContext.begin(), ctx->userContext.end(), nullptr);
        slot != ctx->userContext.end()) {
        *slot = new UserContext{{}, ctx, userId};

        return (libCtxId << 16) | std::ranges::distance(ctx->userContext.begin(), slot);
    }
...
s32 PS4_SYSV_ABI sceNpWebApiCreateContextA(s32 libCtxId,
                                           Libraries::UserService::OrbisUserServiceUserId userId) {
    LOG_ERROR(Lib_NpWebApi, "called, libCtxId = {}, userId = {}", libCtxId, userId);
...
s32 PS4_SYSV_ABI sceNpWebApiCreateRequest(s32 userCtxId, char* api, char* path,
    ...
    if (content && content->len != 0 && content->type == nullptr) {
        return ORBIS_NP_WEBAPI_ERROR_INVALID_CONTENT_PARAMETER;
    }

    auto libCtxId = userCtxId >> 16;
    if (libCtxId < 0 || libCtxId >= MAX_LIB_CONTEXTS) {
        return ORBIS_NP_WEBAPI_ERROR_INVALID_LIB_CONTEXT_ID;
    }
    auto ctx = libContext[libCtxId];
    ...
    {
        std::scoped_lock lk{ctx->mutex};
        auto userCtxId_ = userCtxId & 0xFFFF;
    ...
    if (auto slot = std::ranges::find(userCtx->requests.begin(), userCtx->requests.end(), nullptr);
        slot != userCtx->requests.end()) {
        *slot = new Request{
            {},
            userCtx,
            api,
        ...
        auto req = std::ranges::distance(userCtx->requests.begin(), slot);
        if (reqId) {
            *reqId = static_cast<u64>(userCtxId) << 32 | req;
            LOG_DEBUG(Lib_NpWebApi, "*reqId = {:#x}", *reqId);
        }

        return ORBIS_OK;
    }

    return ORBIS_NP_WEBAPI_ERROR_OUT_OF_MEMORY;
}
...
s32 PS4_SYSV_ABI sceNpWebApiDeleteRequest(s64 reqId) {
    LOG_ERROR(Lib_NpWebApi, "(STUBBED) called, reqId = {:#x}", reqId);
    ...
    delete req;
    userCtx->requests[request] = nullptr;
...
s32 PS4_SYSV_ABI sceNpWebApiInitialize(int libHttpCtxId, u64 poolSize) {
    LOG_ERROR(Lib_NpWebApi, "called, libHttpCtxId = {}, poolSize = {}", libHttpCtxId, poolSize);

    std::scoped_lock lk{lib_mutex};
    if (auto slot = std::ranges::find(libContext.begin(), libContext.end(), nullptr);
        slot != libContext.end()) {
        *slot = new LibraryContext{};

        return std::ranges::distance(libContext.begin(), slot);
    }
...
s32 PS4_SYSV_ABI sceNpWebApiReadData(s64 reqId, void* data, u64 len) {
    ...
    std::scoped_lock reqLk{req->mutex};
    if (!req->response) {
...
s32 PS4_SYSV_ABI sceNpWebApiSendRequest2(s64 reqId, const void* data, u64 len,
    ...
    std::scoped_lock reqLk{req->mutex};
    if (req->client) {
        return ORBIS_NP_WEBAPI_ERROR_AFTER_SEND;
    }
    ...
    if (responseInfo && *(req->response)) {
        responseInfo->statusCode = req->response.value()->status;
    }
